/**
 * Standard Hafs mushaf ayah counts per surah (114 surahs, 6236 ayahs total).
 * Metadata only — no verse text. Used by ingest validation.
 */
...
export const TOTAL_AYAHS = CANONICAL_AYAH_COUNTS.reduce((sum, n) => sum + n, 0);

export function expectedAyahCount(surah: number): number | undefined {
  if (surah < 1 || surah > SURAH_COUNT) return undefined;
  return CANONICAL_AYAH_COUNTS[surah - 1];
}
