...
export function NahjulAudioBar({
  isPlaying,
  isDownloaded,
  isDownloading,
  downloadProgress,
    ...
    >
      ...
      <View style={styles.info}>
        <Text variant="bodySm" color="primary">
          {isPlaying ? t('nahjul.audio.playing') : t('nahjul.audio.listen')}
        </Text>
        {isDownloaded ? (
          <Text variant="caption" color="accent">
            {t('nahjul.audio.offline')}
          </Text>
        ) : isDownloading ? (
          <Text variant="caption" color="secondary">
            {t('nahjul.audio.downloading', { percent: Math.round(downloadProgress * 100) })}
          </Text>
        ) : null}
      </View>
      {isDownloading ? (
        ...
        >
          <Text variant="caption" color={isDownloaded ? 'accent' : 'secondary'}>
            {isDownloaded ? t('nahjul.audio.removeOffline') : '↓'}
          </Text>
        </Pressable>
      )}
    </View>
