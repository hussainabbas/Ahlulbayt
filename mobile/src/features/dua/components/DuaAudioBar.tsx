    ...
    >
      ...
      <View style={styles.info}>
        <Text variant="bodySm" color="primary">
          {isPlaying ? t('dua.audio.playing') : t('dua.audio.listen')}
        </Text>
        {isDownloaded ? (
          ...
          <Text variant="caption" color="secondary">
            {t('dua.audio.downloading', { percent: Math.round(downloadProgress * 100) })}
          </Text>
        ) : null}
      </View>
      ...
      {isDownloading ? (
        ...
        >
          <Text variant="caption" color={isDownloaded ? 'accent' : 'secondary'}>
            {isDownloaded ? t('dua.audio.removeOffline') : '↓'}
          </Text>
        </Pressable>
      )}
    </View>
